...
namespace Arrowgene.Ddon.GameServer.Handler
{
    public class QuestGetCycleContentsStateListHandler : GameRequestPacketHandler<C2SQuestGetCycleContentsStateListReq, S2CQuestGetCycleContentsStateListRes>
    {
        ...
        public override S2CQuestGetCycleContentsStateListRes Handle(GameClient client, C2SQuestGetCycleContentsStateListReq request)
        {
            ...
            var winningPcap = EntitySerializer.Get<S2CQuestFortDefenseNtc>().Read(situation_data0);

            foreach (var cycleContentScheduleId in activeScheduleIds)
            {
                var packet = BuildFortDefenseNtc(Server, cycleContentScheduleId, (ulong)progressEnd.ToUnixTimeSeconds());
                if (packet != null)
                {
                    client.Send(packet);
                }
        ...
        public static S2CQuestFortDefenseNtc? BuildFortDefenseNtc(DdonGameServer server, uint scheduleId, ulong periodEnd = 0)
        {
            var warMissionSchedule = server.GameSettings.Get<Dictionary<uint, (byte Category, uint CategoryType, List<uint> QuestIds)>>("warmission", "WarMissionSchedule");
            var warMissionControl = server.GameSettings.Get<Dictionary<uint, List<(uint QuestId, byte NoticeType)>>>("warmission", "WarMissionControl");

            if (warMissionSchedule == null || warMissionControl == null ||
                !warMissionSchedule.ContainsKey(scheduleId) || !warMissionControl.ContainsKey(scheduleId))
            {
                return null;
            }

            var packet = new S2CQuestFortDefenseNtc();

            foreach (var index in new List<int> { 1, 0 })
            {
                var warmissionMgmtQuestInfo = warMissionControl[scheduleId][index];
                ...
                packet.FortDefenseNoticeDataList.Add(new CDataCycleContentsNoticeData()
                {
                    CycleContentsScheduleId = scheduleId,
                    QuestScheduleId = quest.QuestScheduleId,
                    QuestId = quest.QuestId,
                    CategoryType = warMissionSchedule[scheduleId].CategoryType,
            ...
            byte noticeType = 6;
            foreach (var questId in warMissionSchedule[scheduleId].QuestIds)
            {
